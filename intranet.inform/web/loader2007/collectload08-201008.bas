...
'daystamp=formatdate("YEAR0M0D0H0m0s",daycheck)
	...
	x=scandir(daystamp,"o:/offsite/record/")
	'x=scandir(daystamp,"//everyware-d2/record/")
	x=scandir(daystamp,"o:/record/")
									...
									if doinginsert then
										if filemodifytime(fullord)<(gmtime-180) then
											...
											if not fileready then
												'Was 900 (15mins)
												if filemodifytime(fullord)<(gmtime-10000) then
...
function pingurgent
	postdata="message=URGENT:&port=30001"
