...
<?
  $sqlcount="SELECT date(exporttime),loadagent,loadempty,loadpartial FROM loader07.records" .
  " where serviceid = 'towerhamlets'" .
  " AND servicesub = 'environment'" .
  " AND exporttime >= '20130801000000' " .
  " AND exporttime <= '20130901000000' " .
//  " where serviceid = '{$record['service']}'" .
//  " AND servicesub = '{$record['servsub']}'" .
//  " AND exporttime >= $start " .
//  " AND exporttime <= $end " .
  " GROUP BY date(exporttime)"  ;
