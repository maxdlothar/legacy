...
//2012-07-30 09:50:59
	if (isset($_REQUEST['random'])) {
		...
		$query="SELECT * FROM records WHERE exporttime>DATE_ADD(NOW(), INTERVAL -7 DAY) AND loadempty=0 AND loadexclude=0 AND recorduri like '%t' ORDER BY RAND() LIMIT {$randomcount}";
//		echo $query;
...
//echo $query;
	...
	while ($i < $num) {
		...
		//recformat
			...
			/*.mysql_result($result,$i,'s_primary').mysql_result($result,$i,'ns_primary')
			.' '.mysql_result($result,$i,'f_primary').mysql_result($result,$i,'nf_primary')
			.' '.mysql_result($result,$i,'n_primary').mysql_result($result,$i,'nn_primary')
			.' '.mysql_result($result,$i,'r_primary').mysql_result($result,$i,'ar_primary')
			.' '.mysql_result($result,$i,'a_primary').mysql_result($result,$i,'as_primary')
			.' '.mysql_result($result,$i,'l_primary').mysql_result($result,$i,'al_primary')
			.' '.mysql_result($result,$i,'t_primary').mysql_result($result,$i,'at_primary')
			.' '.mysql_result($result,$i,'u_primary').mysql_result($result,$i,'ac_primary')
			.' '.mysql_result($result,$i,'y_primary')
			.' '.mysql_result($result,$i,'p_primary').mysql_result($result,$i,'ap_primary')
			.' '.mysql_result($result,$i,'e_primary').mysql_result($result,$i,'pp_primary')
			.' '.mysql_result($result,$i,'w_primary').mysql_result($result,$i,'pe_primary')*/
