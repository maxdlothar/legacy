...
function db_select($database, $contest=FALSE) {
  ...
  // Check whether we need to connect to a specific database
  $db_sel=mysql_select_db(DB_DATABASE, $db_con);
  if ($db_sel==FALSE) {
    if ($contest) {
      return FALSE;
    } else {
...
function db_do($sql, $persistent=FALSE, $verbosity=1) {
  ...
  switch($action) {
    case 'SELECT':
    case 'DESCRIBE':
      $data=mysql_query($sql) or db_error("Sorry, but the $action query failed.");
    ...
    case 'COMMIT':
    case 'ROLLBACK':
      $results=mysql_query($sql) or db_error("Sorry, but the $action query failed.");
      break;
    default:
